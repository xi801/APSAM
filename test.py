...
from utils.data_voc import VOCSegmentation,JointTransform2D, ImageToImage2D
...
def main():
    ...
    parser.add_argument('--modelname', default='APSAM', type=str, help='type of model, e.g., SAM, SAMFull, SAMHead, MSA, SAMed, SAMUS...')
    ...
    val_dataset = VOCSegmentation(opt.data_path, opt.val_split, tf_val, img_size=args.encoder_input_size)
    ...
    # checkpoint = torch.load('checkpoints/sam_vit_b_01ec64.pth')
...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    cnn_input = torch.randn(1, 2, args.encoder_input_size, args.encoder_input_size).cuda()
    points = (torch.tensor([[[2, 2]]]).float().cuda(), torch.tensor([[1]]).float().cuda())
    if args.modelname == 'APSAM':
        flops, params = profile(model, inputs=(input, cnn_input, points), )
    else:
    ...
    if opt.mode == "train":
        ...
        # with torch.no_grad():
        #     for image_name,(images,_) in valloader:
