...
from utils.data_voc import JointTransform2D,ImageToImage2D,VOCSegmentation
...
def main():
    ...
    parser.add_argument('--modelname', default='APSAM', type=str, help='type of model, e.g., SAM, SAMFull, MedSAM, MSA, SAMed, SAMUS,APSAM...')
    ...
    parser.add_argument('--task', default='Xrayall', help='task or dataset name')
    ...
    train_dataset = VOCSegmentation(opt.data_path, opt.train_split, tf_train, img_size=args.encoder_input_size)
    val_dataset = VOCSegmentation(opt.data_path, opt.val_split, tf_val, img_size=args.encoder_input_size)  # return image, mask, and filename
    ...
    model.to(device)
    if opt.pre_trained:
        print("loading pre-trained model from {}".format(opt.load_path))
        checkpoint = torch.load(opt.load_path)
        new_state_dict = {}
        for k,v in checkpoint.items():
            if k[:7] == 'module.':
    ...
    # Unfreeze mask_decoder parameters
    ...
    best_miou,best_dice, loss_log, dice_log, iou_log = 0.0, 0.0, np.zeros(opt.epochs+1), np.zeros(opt.epochs+1), np.zeros(opt.epochs+1)
    for epoch in range(opt.epochs):
        #  --------------------------------------------------------- training ---------------------------------------------------------
    
        model.train()
        train_losses = 0
        for batch_idx, (datapack) in enumerate(tqdm(trainloader, desc=f"Epoch {epoch+1}/{opt.epochs}")):
            if batch_idx % 100 == 0:
                tqdm.write(f"Batch {batch_idx+1}/{len(trainloader)}, Train Loss: {train_losses / (batch_idx + 1):.4f}")
            # tqdm.write(f"Batch {batch_idx+1}/{len(trainloader)}, Train Loss: {train_losses / (batch_idx + 1):.4f}", end='\r')
            ...
            cnn_input = datapack['gray_concat'].to(dtype=torch.float32, device=opt.device)
            # -------------------------------------------------------- forward --------------------------------------------------------
            if args.modelname == 'APSAM':
                pred = model(imgs, cnn_input, pt, bbox)
            else:
            ...
            # print(train_loss)
            # if iter_num % 100 == 0:
            #     print(f"Epoch [{epoch+1}/{opt.epochs}] | Iteration [{iter_num}] | Train Loss: {train_loss.item()} |")
        ...
        if epoch % opt.eval_freq == 0:
            ...
            print('epoch [{}/{}], Dice:{:.4f}'.format(epoch, opt.epochs, mean_dice))
            print('epoch [{}/{}], mIoU:{:.4f}'.format(epoch, opt.epochs, mean_iou))
            print('epoch [{}/{}], mAcc:{:.4f}'.format(epoch, opt.epochs, mean_acc))
            if args.keep_log:
                TensorWriter.add_scalar('val_loss', val_losses, epoch)
                TensorWriter.add_scalar('dices', mean_dice, epoch)
                TensorWriter.add_scalar('mIoU', mean_iou, epoch)
                dice_log[epoch] = mean_dice
                iou_log[epoch] = mean_iou
            ...
            if mean_iou > best_miou:
                ...
                if not os.path.isdir(opt.save_path):
                    os.makedirs(opt.save_path)
                save_path = opt.save_path + args.modelname + opt.save_path_code + '%s' % timestr + '_' + str(epoch) + '_' + str(best_miou)
                torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)
        if epoch % opt.save_freq == 0 or epoch == (opt.epochs-1):
            ...
            if args.keep_log:
                ...
                with open(opt.tensorboard_path + args.modelname + opt.save_path_code + logtimestr + '/iou.txt', 'w') as f:
                    for i in range(len(iou_log)):
                        f.write(str(iou_log[i])+'\n')
