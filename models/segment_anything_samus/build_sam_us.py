...
from models.segment_anything_samus.modeling import apsam

from .modeling import ImageEncoderViT_sam, ImageEncoderViT, MaskDecoder, PromptEncoder, Samus, TwoWayTransformer,ImageEncoderViT_CNN,MaskDecoder_token
from torch.nn import functional as F

from .modeling.apsam import APSAM
...
def build_samus_vit_b_norefine(args, checkpoint=None):
    return _build_samus_norefine(
        args,
        encoder_embed_dim=768,
        encoder_depth=12,
        encoder_num_heads=12,
...
def build_apsam_vit_b(args, checkpoint=None):
    return _build_apsam(
        args,
        encoder_embed_dim=768,
        encoder_depth=12,
...
samus_model_registry = {
    ...
    "vit_b_no_refine":build_samus_vit_b_norefine,
    "vit_b_apsam":build_apsam_vit_b,
}
...
def _build_samus_norefine(
    ...
    image_embedding_size = image_size // patch_size
    samus = Samus(
        image_encoder=ImageEncoderViT_sam(
            depth=encoder_depth,
            embed_dim=encoder_embed_dim,
            img_size=image_size,
            mlp_ratio=4,
...
def _build_apsam(
    ...
    image_embedding_size = image_size // patch_size
    samus = APSAM(
        image_encoder=ImageEncoderViT_CNN(
            depth=encoder_depth,
            embed_dim=encoder_embed_dim,
            img_size=image_size,
            mlp_ratio=4,
        ...
        mask_decoder=MaskDecoder_token(
            num_multimask_outputs=3,
            transformer=TwoWayTransformer(
                depth=2,
                embedding_dim=prompt_embed_dim,
