...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(tqdm(valloader, desc="Evaluating")):
        imgs = Variable(datapack['image'].to(dtype = torch.float32, device=opt.device))
        masks = Variable(datapack['low_mask'].to(dtype = torch.float32, device=opt.device))
        label = Variable(datapack['label'].to(dtype = torch.float32, device=opt.device))
        class_id = datapack['class_id']
        image_filename = datapack['image_name']
        cnn_input = Variable(datapack['gray_concat'].to(dtype=torch.float32, device=opt.device))
        pt = get_click_prompt(datapack, opt)

        with torch.no_grad():
            start_time = time.time()
            if args.modelname == 'APSAM':
                pred = model(imgs, cnn_input, pt)
            else:
                pred = model(imgs, pt)
            sum_time =  sum_time + (time.time()-start_time)
        ...
        eval_number = eval_number + b
        test_iou = np.mean(ious*100, axis=0)
        # print(test_iou[1])
        # print(ious.shape)
        # print(accs.shape)
        # print(hds.shape)
    ...
    dice_mean = np.mean(dices, axis=0)*100
    dices_std = np.std(dices, axis=0)
    hd_mean = np.mean(hds, axis=0)*100
    hd_std = np.std(hds, axis=0)
    ...
    iou_mean = np.mean(ious, axis=0)*100
    iou_std = np.std(ious, axis=0)
    acc_mean = np.mean(accs, axis=0)*100
    acc_std = np.std(accs, axis=0)



    # mean_dice = np.mean(dice_mean*100,axis=0)
    # mean_hdis = np.mean(hd_mean*,axis=0)
    # mean_iou = np.mean(iou_mean,axis=0)
    # mean_acc = np.mean(acc_mean,axis=0)
    print("test speed", eval_number/sum_time)
    if opt.mode == "train":
        # return dices, mean_dice, mean_hdis, val_losses
        return dices, dice_mean[1], hd_mean[1], iou_mean[1], acc_mean[1], val_losses
    else:
...
def eval_camus_patient(valloader, model, criterion, opt, args):
    ...
    mean_iou = np.mean(iou_mean[1:], axis=0)
    mean_acc = np.mean(acc_mean[1:], axis=0)
    #return dices, mean_dice, val_losses
    if opt.mode=="train":
        return dices, mean_dice, mean_iou, mean_acc, mean_hdis, val_losses
    else:
    ...
    for batch_idx, (datapack) in enumerate(tqdm(valloader, desc="Evaluating")):
        ...
        for j in range(0, b):
            for c in range(1, opt.classes):
                pred_i = np.zeros((1, h, w))
                pred_i[seg[j:j+1, :, :] == c] = 255
                gt_i = np.zeros((1, h, w))
                gt_i[gt[j:j+1, :, :] == c] = 255
                dices[eval_number+j, c] += metrics.dice_coefficient(pred_i, gt_i)
                iou, acc, se, sp = metrics.sespiou_coefficient2(pred_i, gt_i, all=False)
                ious[eval_number+j, c] += iou
                accs[eval_number+j, c] += acc
                ses[eval_number+j, c] += se
                sps[eval_number+j, c] += sp
                hds[eval_number+j, c] += hausdorff_distance(pred_i[0, :, :], gt_i[0, :, :], distance="manhattan")
                del pred_i, gt_i
                if opt.visual:
                    visual_segmentation_sets_with_pt(seg[j:j+1, :, :], image_filename[j], opt, pt[0][j, :, :])
            # pred_i = np.zeros((1, h, w))
