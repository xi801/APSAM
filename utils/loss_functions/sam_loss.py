...
class DiceLoss(nn.Module):
    ...
    #     # if torch.any(input_tensor < 0) or torch.any(input_tensor >= self.n_classes):
    #     #     raise ValueError(f"Input tensor contains values outside the range [0, {self.n_classes-1}]")
    #     tensor_list = []
    ...
    def _one_hot_encoder(self, input_tensor):
        # if torch.any(input_tensor < 0) or torch.any(input_tensor >= self.n_classes):
        #     raise ValueError(f"Input tensor contains values outside the range [0, {self.n_classes-1}]")
        # temp_prob = (input_tensor != 0).float()
        # output_tensor = temp_prob.unsqueeze(1) # b h w -> b 1 h w
        # return output_tensor.float()

        tensor_list = []
        for i in range(self.n_classes):
            temp_prob = (input_tensor == float(i)).float()
            # unique_values = torch.unique(temp_prob)
            # print(f"Unique values in temp_prob for class {i}: {unique_values}")
            # print(temp_prob)
            tensor_list.append(temp_prob.unsqueeze(1)) # b h w -> b 1 h w
        output_tensor = torch.cat(tensor_list, dim=1)
        return output_tensor.float()
...
class Mask_DC_and_BCE_loss(nn.Module):
    def __init__(self, pos_weight, dice_weight=0.8):
        ...
        # self.ce = torch.nn.CrossEntropyLoss(weight=pos_weight)
...
def get_criterion(modelname='SAM', opt=None):
    ...
    #     criterion = DC_and_BCE_loss(classes=13, dice_weight=0.8)
    else:
        criterion = Mask_DC_and_BCE_loss(pos_weight=pos_weight)
    return criterion
