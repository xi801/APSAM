...
def visual_segmentation_sets_with_pt(seg, image_filename, opt, pt):
    ...
    # img_ori = cv2.imread(os.path.join(opt.data_subpath + '/img', image_filename))
    # img_ori0 = cv2.imread(os.path.join(opt.data_subpath + '/img', image_filename))
    img_path = os.path.join(opt.data_subpath, image_filename)
    img_ori = cv2.imread(os.path.join(opt.data_subpath, image_filename))
    img_ori0 = cv2.imread(os.path.join(opt.data_subpath, image_filename))
...
def infer_and_save_masks(model, test_loader, opt):
    model.eval()  # 设置模型为评估模式
    with torch.no_grad():  # 禁用梯度计算
        for image_filename, (images, _) in test_loader:  # 遍历测试集
            images = images.to(opt.device)
            # 预测分割结果
            seg = model(images)
            
            # 将预测结果转换为适当的形式
            seg = seg.argmax(dim=1)  # 假设是分类任务，取最大类别作为mask
            
            # 可视化并保存分割图像
            visual_segmentation(seg.cpu(), image_filename, opt)